...
function displayDollarWidget(oficial, blue) {
    ...
    const navContainer = nav.querySelector('.max-w-7xl .flex.justify-between');
    ...
    widget.className = 'hidden md:flex items-center gap-3 px-4 py-2 bg-gray-100 dark:bg-gray-700 rounded-lg text-sm';
    
    let html = '<div class="flex items-center gap-3">';
    html += '<span class="text-gray-600 dark:text-gray-400 font-medium">💵 USD/ARS:</span>';
    
    if (oficial) {
        html += `
            <div class="flex flex-col">
                <span class="text-xs text-gray-500 dark:text-gray-400">Oficial</span>
    ...
    // Add close button
    html += `
        <button onclick="closeDollarWidget()" class="ml-2 p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors" title="Cerrar">
    ...
    // Insert widget before the right section (dark mode toggle, etc.)
    const rightSection = navContainer.querySelector('.flex.items-center.gap-4');
    if (rightSection) {
        navContainer.insertBefore(widget, rightSection);
    }
}
